...
![Pasted-image-20221228114022.png](/Images/Pasted-image-20221228114022.png)
...
![Pasted-image-20221228114037.png](/Images/Pasted-image-20221228114037.png)
...
```shell-session
ChefBaki@htb[/htb]$ curl -s https://crt.sh/\?q\=inlanefreight.com\&output\=json | jq .
...
```shell-session
ChefBaki@htb[/htb]$ curl -s https://crt.sh/\?q\=inlanefreight.com\&output\=json | jq . | grep name | cut -d":" -f2 | grep -v "CN=" | cut -d'"' -f2 | awk '{gsub(/\\n/,"\n");}1;' | sort -u
...
```shell-session
ChefBaki@htb[/htb]$ for i in $(cat subdomainlist);do host $i | grep "has address" | grep inlanefreight.com | cut -d" " -f1,4;done
...
```shell-session
ChefBaki@htb[/htb]$ for i in $(cat subdomainlist);do host $i | grep "has address" | grep inlanefreight.com | cut -d" " -f4 >> ip-addresses.txt;done
ChefBaki@htb[/htb]$ for i in $(cat ip-addresses.txt);do shodan host $i;done
...
```shell-session
ChefBaki@htb[/htb]$ dig any inlanefreight.com
...
![Pasted-image-20221228114201.png](/Images/Pasted-image-20221228113610.png)
