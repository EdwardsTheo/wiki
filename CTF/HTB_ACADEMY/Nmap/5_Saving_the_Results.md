...
```shell-session
ChefBaki@htb[/htb]$ sudo nmap 10.129.2.28 -p- -oA target
...
```shell-session
ChefBaki@htb[/htb]$ cat target.nmap
...
```shell-session
ChefBaki@htb[/htb]$ cat target.gnmap
...
```shell-session
ChefBaki@htb[/htb]$ cat target.xml
...
```shell-session
ChefBaki@htb[/htb]$ xsltproc target.xml -o target.html
```
